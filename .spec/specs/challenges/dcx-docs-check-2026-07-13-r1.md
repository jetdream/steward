...
- [severity: low] .spec/CLAUDE.md folder map still says "architecture/ — … (sketch status until code lands)", contradicting DCX-11 and architecture/CLAUDE.md (approving sketches is the *first* task of the design pass, a hard predecessor of spec approval). One-line router fix; low because the more specific router already states the correct semantics.
