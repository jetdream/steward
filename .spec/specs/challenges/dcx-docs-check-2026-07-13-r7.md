...
2. DCX-3 v5 exemption binding: the r6 evil-note.md attack (challenge-record frontmatter in .spec/product/ with ZZZ-999, MEM-999, `G-1 v99`) now produces 4 errors — all three reference/pin errors fire normally, its citation reappears in the DCX-5 cascade list, and the DCX-4 placement error fires.
3. DCX-13 v3 path prefix: the r6 smuggled-record attack (record pointed at .spec/product/) now errors "challenge record must live under .spec/specs/challenges/", exit 1.
