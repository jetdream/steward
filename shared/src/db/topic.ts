 ...
 *
 * SCOPE — description + why-it-fits + the resolvable evidence pointer (grounding
 * guard) + provenance + status + the supersession pair (evolution, VAL-3, like
 * memory_entry) + the TOPS-2 research-strategy package (nullable until authored;
 * the Radar EXT-1 discovers against it).
 *
...
import type { EvidencePointer, ResearchStrategy } from "../entities/topic.js";
...
export const topic = pgTable(
  "topic",
  {
    ...
    /** The TOPS-2 research-strategy package the Radar discovers against (null until authored). */
    researchStrategy: jsonb("research_strategy").$type<ResearchStrategy>(),
